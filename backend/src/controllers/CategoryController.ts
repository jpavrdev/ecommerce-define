...
export async function childrenWithCount(req: Request, res: Response) {  const parentId = Number(req.params.id);  if (!Number.isFinite(parentId)) return res.status(400).json({ message: 'Categoria inv�lida' });  try {    const rows = await Category.findAll({      where: { parentId },      attributes: {        include: [          [            sequelize.literal('(SELECT COUNT(*) FROM products p WHERE p.categoryId = Category.id)'),            'productCount',          ],        ],      },      order: [[sequelize.literal('productCount'), 'DESC'], ['name', 'ASC']],    });    return res.json(rows);  } catch (err) {    return res.status(500).json({ message: 'Erro ao listar subcategorias', error: (err as Error).message });  }}
